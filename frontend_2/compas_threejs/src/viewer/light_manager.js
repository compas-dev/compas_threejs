...
export const SCENE_LIGTH_HELPERS = {};
export function lightManager(data) {
...
function buildPointLight(data) {
    let pointLight, helper;
    // Get the light
    if (SCENE_LIGHTS[data.guid.value]) {
        pointLight = SCENE_LIGHTS[data.guid.value];
    }
    else {
...
function buildSpotLight(data) {
    ...
    // Get the light
    if (SCENE_LIGHTS[data.guid.value]) {
        spotLight = SCENE_LIGHTS[data.guid.value];
...
function buildRectLight(data) {
    let rectLight, helper;
    // Get the light
    if (SCENE_LIGHTS[data.guid.value]) {
        rectLight = SCENE_LIGHTS[data.guid.value];
    }
    else {
...
function buildSunlight(data) {
    ...
    // Get the light
    if (SCENE_LIGHTS[data.guid.value]) {
        sunlight = SCENE_LIGHTS[data.guid.value];
    }
    else {
...
function buildSky(data) {
    let sky, sun, ambient;
    if (SCENE_LIGHTS[data.guid.value]) {
        sky = SCENE_LIGHTS[data.guid.value];
...
function getSunColor(elevation) {
    if (elevation > 10)
        return new THREE.Color(0xffffff); // White sun when high
    if (elevation > 0) {
...
function buildAmbientLight(data) {
    let ambientLight;
    // Get the light
    if (SCENE_LIGHTS[data.guid.value]) {
        ambientLight = SCENE_LIGHTS[data.guid.value];
    }
    else {
