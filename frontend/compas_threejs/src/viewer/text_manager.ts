...
export function textManager(data: { [key: string]: any }) {
...
const FONT_CACHE: { [key: string]: any } = {};

async function loadFont(fontName: string, fontWeight: string): Promise<any> {
...
async function buildTextGeometry(data: { [key: string]: any }) {
  ...
  const transformationMatrix = new THREE.Matrix4().makeBasis(
    xAxis,
    yAxis,
    zAxis,
  );
